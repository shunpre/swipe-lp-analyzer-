...
# タブ10: 使用ガイド
    ...
    #### **1. 分析の基本フロー**
    本ツールは、以下のフローでご活用いただくことで、LPの課題発見から改善策の立案までをシームレスに支援します。
    1.  **全体サマリーの確認**: まず「全体サマリー」でLP全体の健康状態（主要KPI）を把握します。特にコンバージョン率（CVR）、FV残存率、最終CTA到達率の3点に注目します。
    2.  **課題領域の特定**: 全体サマリーでパフォーマンスが低い指標が見つかった場合、関連する詳細分析ページで深掘りします。
        -   **例1**: CVRが低い → 「セグメント分析」で特定のデバイスやチャネルに問題がないか確認します。
        -   **例2**: FV残存率が低い → 「ページ分析」でファーストビュー（ページ1）の離脱要因を探ります。
    3.  **ボトルネックの発見**: 「ページ分析」のポジショニングマップや離脱率ランキングから、ユーザーが最も離脱しているボトルネックページを特定します。
    4.  **改善仮説の立案**: 特定した課題に対し、「AIによる分析・考察」ページで目標値や課題を入力し、具体的な改善アイデアを得ます。
    5.  **施策の実行と効果測定**: AIの提案を参考にA/Bテストなどの改善施策を実行し、再度本ツールで効果を測定します。
    ...
    ##### **AIアナリスト**
    -   **AIによる分析・考察**:
        -   **目的**: LP全体のデータとユーザーが入力した目標値・課題を基に、AIがオーダーメイドの改善戦略を立案します。
        -   **活用シーン**:
            -   どこから手をつければ良いか分からない時。
            -   具体的な改善アイデアやA/Bテストの案が欲しい時。
            -   分析結果を基にしたレポートや提案書を作成する時。

    ##### **基本分析**
    -   **リアルタイムビュー**:
        -   **目的**: 直近1時間のサイト活動を監視し、突発的な変化を即座に検知します。
        -   **活用シーン**:
            -   広告キャンペーン開始直後の効果測定。
            -   メディア掲載やSNSでの拡散によるトラフィック急増の把握。
            -   サイト障害や広告配信停止などの異常の早期発見。
    -   **全体サマリー**:
        -   **目的**: LP全体のパフォーマンスを俯瞰し、主要KPIの現状を網羅的に把握します。
        -   **活用シーン**:
            -   日次・週次でのパフォーマンス定点観測。
            -   LP全体の健康状態のチェック。
            -   詳細分析に入る前の、課題領域のあたりつけ。
    -   **時系列分析**:
        -   **目的**: 曜日・時間帯ごとのパフォーマンス傾向を分析し、最適なタイミングを発見します。
        -   **活用シーン**:
            -   広告配信スケジュールの最適化（CVRが高い時間帯に入札を強化するなど）。
            -   メールマガジン配信やSNS投稿の最適なタイミングの特定。
    -   **デモグラフィック情報**:
        -   **目的**: ユーザーの属性（年齢、性別、地域など）ごとの行動の違いを分析し、ターゲット顧客の解像度を高めます。
        -   **活用シーン**:
            -   広告のターゲティング精度の向上。
            -   ペルソナ（顧客像）の再定義や具体化。
            -   セグメントに合わせたコンテンツパーソナライズの検討。

    ##### **LP最適化分析**
    -   **ページ分析**:
        -   **目的**: ページ単位でユーザー行動を可視化し、離脱の主要因となっているボトルネックページを特定します。
        -   **活用シーン**:
            -   LPのどこでユーザーが離脱しているかを特定したい時。
            -   コンテンツの流れや構成に問題がないかを確認したい時。
            -   改善インパクトが最も大きいページを見つけたい時。
    -   **A/Bテスト分析**:
        -   **目的**: 実施したA/Bテストの結果を統計的に評価し、どちらのパターンが優れているかを客観的に判断します。
        -   **活用シーン**:
            -   テスト結果の勝敗判定。
            -   結果が偶然ではないか（統計的に有意か）の確認。
            -   次のテスト計画のためのインサイト獲得。

    ##### **詳細分析**
    -   **セグメント分析**:
        -   **目的**: デバイス（PC/スマホ）やチャネル（流入経路）などのセグメント別にデータを深掘りし、パフォーマンス差異の原因を探ります。
        -   **活用シーン**:
            -   「スマホでのCVRだけが低い」など、特定のセグメントにおける課題を発見したい時。
            -   効果の高い広告チャネルや、逆に非効率なチャネルを特定したい時。
    -   **インタラクション分析**:
        -   **目的**: CTAボタンやリンクなど、クリック可能な要素のパフォーマンスを分析し、ユーザーの関心事を特定します。
        -   **活用シーン**:
            -   どのCTAボタンが最もクリックされているかの把握。
            -   クリック率が低い要素のデザインや文言の改善検討。
    -   **動画・スクロール分析**:
        -   **目的**: 動画の視聴状況やユーザーのスクロール行動を分析し、コンテンツのエンゲージメントを評価します。
        -   **活用シーン**:
            -   動画コンテンツがコンバージョンに貢献しているかの評価。
            -   ユーザーがどこまでコンテンツを読み進めているかの把握。
            -   スクロールされない、または逆行が多いページの問題点特定。

    ##### **ヘルプ**
    -   **使用ガイド**: このページです。ツールの機能と活用方法について解説します。
    -   **専門用語解説**: 分析レポートで使用される主要なマーケティング・分析用語について解説します。
    """)
